...
헌금 배치·결산·홈택스 자동 전송 API, 실제 중복 병합, 교적 증명서는 후속 기능이다.
...
Phase 3E 연간 예산: 지출 계정·기금별 편성, 불변 변경 이력, 원장 집행률·잔여액,
예산 초과/미편성 표시와 분리된 조회·편성 권한을 구현했다. [Phase 3E](phase-3e-annual-budgets.md)와 ADR-011을 따른다.

Phase 3F 예산 변경 독립 승인·반려·취소와 지출 예약·WARN/BLOCK 통제를 구현했다.
연도와 기존 자료 이행, 권한·검증 범위는 [Phase 3F](phase-3f-budget-controls.md), ADR-012를 따른다.
