...
budget.approve는 budget.read와 함께 검사하며 작성자와 다른 승인자만 결정한다. 운영 MFA·최근 인증이 필요하다.
finance.manage의 예산 통제 설정 변경도 최근 인증을 요구한다. 지출 참여자에게 교회 전체 예산 금액을 노출하지 않고 적합 상태만 반환한다.
