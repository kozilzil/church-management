...
BEGIN;
INSERT INTO app_user(id,church_id,username,active) VALUES ('00000000-0000-4000-8000-000000000006','00000000-0000-4000-8000-000000000001','synthetic-budget-approver',false);
INSERT INTO budget_change(id,church_id,year,account_id,fund_id,base_version,amount,reason,requested_by) VALUES ('00000000-0000-4000-8000-000000000007','00000000-0000-4000-8000-000000000001',2020,'00000000-0000-4000-8000-000000000003','00000000-0000-4000-8000-000000000004',0,12345,'Synthetic restore probe','00000000-0000-4000-8000-000000000002');
INSERT INTO budget_decision(change_id,church_id,decision,reason,decided_by) VALUES ('00000000-0000-4000-8000-000000000007','00000000-0000-4000-8000-000000000001','APPROVED','Synthetic approval','00000000-0000-4000-8000-000000000006');
INSERT INTO budget_revision(id,church_id,year,account_id,fund_id,version,amount,reason,created_by,change_id) VALUES ('00000000-0000-4000-8000-000000000005','00000000-0000-4000-8000-000000000001',2020,'00000000-0000-4000-8000-000000000003','00000000-0000-4000-8000-000000000004',1,12345,'Synthetic restore probe','00000000-0000-4000-8000-000000000002','00000000-0000-4000-8000-000000000007');
INSERT INTO budget_policy_revision(id,church_id,version,mode,reason,created_by) VALUES ('00000000-0000-4000-8000-000000000008','00000000-0000-4000-8000-000000000001',1,'BLOCK','Synthetic blocking policy','00000000-0000-4000-8000-000000000006');
COMMIT;
SQL
...
verify_budget_probe() {
  compose exec -T api node -e 'const {PrismaClient}=require("./apps/api/node_modules/@prisma/client");const db=new PrismaClient();(async()=>{const r=await db.budgetRevision.findUniqueOrThrow({where:{id:"00000000-0000-4000-8000-000000000005"}});if(r.amount.toFixed(0)!=="12345"||r.version!==1)throw Error("Budget restore probe changed");const d=await db.budgetDecision.findUniqueOrThrow({where:{changeId:r.changeId}});if(d.decision!=="APPROVED")throw Error("Budget approval not restored");const policy=await db.budgetPolicyRevision.findUniqueOrThrow({where:{id:"00000000-0000-4000-8000-000000000008"}});if(policy.mode!=="BLOCK")throw Error("Budget control policy not restored");for(const table of ["budget_revision","budget_change","budget_decision","budget_policy_revision","expense_budget_check"]){const [p]=await db.$queryRawUnsafe("SELECT has_table_privilege(current_user,$1,$2) AS edit,has_table_privilege(current_user,$1,$3) AS remove,has_table_privilege(current_user,$1,$4) AS create",table,"UPDATE","DELETE","INSERT");if(p.edit||p.remove||!p.create)throw Error("Budget runtime privileges incorrect")}})().catch(e=>{console.error(e.message);process.exitCode=1}).finally(()=>db.$disconnect())'
}
verify_budget_probe
